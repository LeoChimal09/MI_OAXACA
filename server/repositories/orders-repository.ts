...
function generateOrderRef() {
  return `MIO-${randomBytes(8).toString("hex").toUpperCase()}`;
}
...
function toPlacedOrder(row: DbOrderRow): PlacedOrder {
  return {
    ...
    paymentProvider: (row.paymentProvider as "stripe" | null) ?? null,
    ...
    cancelledBy: (row.cancelledBy as CancellationActor | null) ?? null,
  ...
  const next = {
    ...
    paymentCurrency:
      input.paymentCurrency !== undefined
        ...
        : (existing.paymentCurrency ?? null),
    paymentAmountCents:
      input.paymentAmountCents !== undefined
        ? input.paymentAmountCents
        : (existing.paymentAmountCents ?? null),
...
export async function updateOrderStatus(
  ...
  // Simple validation: can transition to new status
  const nextEta = status === "in_progress" ? (options?.etaMinutes ?? existing.etaMinutes ?? null) : existing.etaMinutes ?? null;
